...
#include "WallAnalyzer.hpp"


WallAnalyzer::WallAnalyzer(Maze& m) : Analyzer( m )
{
    Update(m);
}


WallAnalyzer::~WallAnalyzer()
{

}


void WallAnalyzer::Update(Maze& m)
{
    for(int y = 0; y < h; y += 1)
    {
        for(int x = 0; x < w; x += 1)
        {
            if( (m.tileType(x, y) != T_OBSTACLE) && (m.tileType(x, y) != T_CADRE) )
            {
                texture[x + w * y] = m.tileType(x, y);
                continue;
            }
            const bool up       = (m.tileType(x+0, y-1) == T_OBSTACLE) || (m.tileType(x+0, y-1) == T_CADRE);
            const bool left     = (m.tileType(x-1, y+0) == T_OBSTACLE) || (m.tileType(x-1, y+0) == T_CADRE);
            const bool right    = (m.tileType(x+1, y+0) == T_OBSTACLE) || (m.tileType(x+1, y+0) == T_CADRE);
            const bool down     = (m.tileType(x+0, y+1) == T_OBSTACLE) || (m.tileType(x+0, y+1) == T_CADRE);
